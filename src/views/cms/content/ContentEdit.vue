...
<script>
  ...
  export default {
      ...
      loadEditData (id) {
        ...
        self.$http.get('/cms/content/' + self.id)
          .then(function (response) {
            ...
            self.mysetAttribute(response.data.data.video, self.form.video)
            self.form.video.player = response.data.data.video.player
            self.form.videoOtherPlayer = response.data.data.videoOtherPlayer || []
    ...
    beforeRouteEnter  (to, from, next) {
      next(vm => {
        // 通过 `vm` 访问组件实例
        let dataControl = 'ContentEditLoadData=true'
