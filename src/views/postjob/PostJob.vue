  ...
  export default {
    ...
    methods: {
      ...
      loadTableData (pageNo, pageNoChange) {
        ...
        this.$http.get('/base/postjob/postjobs', self.searchFormModel)
      ...
      editTableRowClick (index, row) {
        this.$utils.loadDataControl.add('PostJobEditLoadData=true')
        this.$router.push('/Main/PostJobEdit/' + row.id)
      },
      // tablb 表格删除行
      deleteTableRowClick (index, row) {
        ...
        this.$confirm('确定要删除吗, 是否继续?', '提示', {
          ...
          this.$http.delete('/base/postjob/postjob/' + row.id)
            .then(function (response) {
      ...
      addTableRowClick () {
        this.$utils.loadDataControl.add('PostJobAddLoadData=true')
        this.$router.push('/Main/PostJobAdd')
      },
      resetFormClick () {
