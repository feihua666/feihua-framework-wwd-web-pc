  ...
  export default {
    ...
    methods: {
      loadEditData (id) {
        ...
        self.$http.get('/base/postjob/postjob/' + self.id)
          .then(function (response) {
            ...
            self.form.dataType = content.dataType
              ...
              self.$http.put('/base/postjob/postjob/' + self.id, self.form)
                ...
                .catch(function (response) {
                  if (response.response.status === 404) {
                    self.$message.error('岗位职务改失败，数据不存在或已被他人修改，请刷新列表后再试')
                  }
                  self.addLoading = false
                })
            } else {
    ...
    beforeRouteEnter  (to, from, next) {
      next(vm => {
        // 通过 `vm` 访问组件实例
        let dataControl = 'PostJobEditLoadData=true'
